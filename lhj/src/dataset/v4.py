...
def get_train_test_df(result_dir: str = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    total_df["day_sin"] = np.sin(2 * np.pi * total_df["day"] / 31)
    total_df["day_cos"] = np.cos(2 * np.pi * total_df["day"] / 31)

    total_df["weekday"] = total_df["lifelog_date"].dt.weekday
    total_df["weekday_sin"] = np.sin(2 * np.pi * total_df["weekday"] / 7)
    total_df["weekday_cos"] = np.cos(2 * np.pi * total_df["weekday"] / 7)
